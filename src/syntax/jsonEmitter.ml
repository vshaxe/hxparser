...
module JsonEmitter(Api : JsonApi) = struct
	...
	let enum enum_name constructor_name arguments =
		Api.jobject [
			"name",Api.jstring constructor_name;
			"args",Api.jarray arguments;
		]
	...
	let emit_array_expr e1 e2 p =
		enum "Expr" "EArray" [e1;tok;e2;tok]
	...
	let emit_complex_type_parent ct p =
		enum "ComplexType" "Parent" [tok;ct;tok]
	...
	let emit_field_expr_none =
		enum "MethodExpr" "None" [tok]

	let emit_field_expr_block e =
		enum "MethodExpr" "Block" [e]

	let emit_field_expr_expr e =
		enum "MethodExpr" "Expr" [e;tok]
	...
	let emit_constraints_single ct =
		enum "Constraints" "Single" [tok;ct]

	let emit_constraints_multiple ctl =
		enum "Constraints" "Multiple" [tok;tok;arr ctl;tok]
